...
#plot cv model
coef(relaxed_cv_model)
plot(relaxed_cv_model)


#use coefficients from cv for model
#create train and test explinatory from cv coefficients
#lasso_train_expl = data.matrix(train_df[,c(4,7,8,9,12,13,14,15)])
#lasso_test_expl = data.matrix(test_df[,c(4,7,8,9,12,13,14,15)])

rlasso_pred = predict(rlasso_cv_model,test_expl,type = 'response')
new_pred<-factor(ifelse(as.numeric(rlasso_pred)>.5,'yes','no' ) ,levels = c('no','yes'))
confusionMatrix(new_pred,test_resp)

plot(new_pred,test_resp)
...
```{r ASSESS RELAXED LASSO MODEL}
#assess lasso model
assess.glmnet(rlasso_cv_model,lasso_test_expl,test_resp, family = 'binomial',s = relaxed_best_lambda)
#return confusion matrix from lasso model
confusion.glmnet(rlasso_cv_model,lasso_test_expl,test_resp,family = 'binomial',s = relaxed_best_lambda)
#plot roc curve from lasso model
plot(roc.glmnet(rlasso_cv_model,lasso_test_expl,test_resp,family = 'binomial',s = relaxed_best_lambda))
