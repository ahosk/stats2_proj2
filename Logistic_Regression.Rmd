...
StepwiseFeature =step(slr,direction="both",trace=0)
summary(StepwiseFeature)

#stepwise feature selection model
slr_fsr = glm(y ~ job+marital+education+balance+housing+loan+contact+day+month+duration+campaign+poutcome, family = 'binomial',data = test_simple)


#predictions from stepwise output 
slrPredict_fsr = predict(slr_fsr,newdata = test_simple,type = 'response')
...
confusionMatrix(slrPredictnew_fsr,test_simple$y)
...
#Pearson Chi^2 Test
sum(residuals(relaxed_cv_model, type = "pearson")^2)
...
anova(slr,relaxed_cv_model, test = "Chisq",class(relaxed_cv_model))
